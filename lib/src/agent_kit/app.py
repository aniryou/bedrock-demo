"""AgentCore Runtime entrypoint factory.

`build_app(spec)` wraps `build_agent(spec, ...)` in a `BedrockAgentCoreApp`, exposing the
`/invocations` and `/ping` endpoints AgentCore Runtime expects, and streams the agent's
output back.
...
TOKEN_EXCHANGE). `BedrockAgentCoreApp` is imported lazily inside `build_app` so that
importing `agent_kit` does not require the `deploy` extra.
"""
...
_LOG = logging.getLogger("agent_kit.app")


def _extract_user_jwt(context, header_name: str) -> str | None:
...
def build_app(spec):
    """Build the AgentCore Runtime app for `spec`.

    Seeds the per-agent config defaults, lazily imports `BedrockAgentCoreApp` (so importing
    `agent_kit` does not pull the `deploy` extra), registers the streaming invoke entrypoint,
    and returns the app.
    """
    config.configure(model_id=spec.model_id, region=spec.region, max_tokens=spec.max_tokens)
        ...
        try:
            ...
            emf = {
                "_aws": {
                    ...
                    "CloudWatchMetrics": [
                        {
                            "Namespace": spec.metric_namespace,
    ...
    async def invoke(payload, context=None):
        ...
        token = identity.set_user_jwt(
            _extract_user_jwt(context, get_config().user_jwt_header)
        )
        try:
            ...
            actor = identity.actor_id(default=spec.agent_id)
