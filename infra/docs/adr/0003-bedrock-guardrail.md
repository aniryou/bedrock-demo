...
Attach a single CLASSIC-tier native Bedrock Guardrail at the model path whose **only policy is the `PROMPT_ATTACK` input filter**. Five labelled sub-decisions, all grounded in the deployed `guardrail.tf` / `runtime.tf` / `iam.tf` / `agent.py` / `config.py`.
...
4. **Spike-vs-deployed drift on trace/stream mode** — SPIKE-GUARDRAILS §4.5 drafted `sync` + `guardrail_trace=enabled`; the deployed `agent.py` uses `async` and omits `guardrail_trace`, so per-block assessment detail is not emitted in-process. *Mitigation:* this is consistent with D2 (nothing to mask inline → async is fine) and D4; per-intervention visibility comes from the vended CloudWatch metrics (ADR-0004), and the model-invocation log is masked for stored PII. Revisit if per-block trace detail is required.
...
4. [x] **D4** — `agent.py` sets `guardrail_stream_processing_mode="async"` + `guardrail_redact_input=False`, gated on both `cfg.guardrail_id` and `cfg.guardrail_version`; `config.py` reads both env vars (default `""`).
