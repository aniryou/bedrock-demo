...
The stack deploys an `aws_bedrockagentcore_memory` resource with **all three long-term strategies** — SEMANTIC (`/facts/{actorId}`), SUMMARIZATION (`/summaries/{actorId}/{sessionId}`), USER_PREFERENCE (`/preferences/{actorId}`) — see `terraform/memory.tf`. The Strands agent consumes memory through `AgentCoreMemorySessionManager` (`../../../agent/src/order_triage/memory.py`). A spike found three gaps:
...
**D2 — `actor_id` = Entra subject.** Derive the memory actor from the **verified inbound JWT's `sub` claim** (fall back to `oid`, then to the anonymous literal `"order-triage"` when no user token is present). The CUSTOM_JWT authorizer has already cryptographically verified the token before the runtime sees it (per ADR-0001), so the runtime decodes the **payload only, without re-verifying the signature**, purely to read the subject. `sub` is an opaque, stable per-(user, app) identifier — it keeps **PII out of the namespace path** (no email/UPN) while giving each user their own `/facts` and `/preferences` partition. `build_session_manager` resolves the actor from the request `identity` contextvar (mirroring how `gateway.py` pulls the bearer), so the anonymous-fallback literal has a **single source** (`identity.ANONYMOUS_ACTOR`) and nothing is threaded through `build_agent`.
...
1. [x] **D1** — `retrieval_config` added to `memory.py` (templated namespaces, `top_k`/`relevance_score`).
2. [x] **D2** — JWT-payload subject decode in `identity.py` (`sub`→`oid`→anonymous); threaded `runtime.py → build_agent → build_session_manager`.
