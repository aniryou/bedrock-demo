...
**D2 — `actor_id` = Entra subject.** Derive the memory actor from the **verified inbound JWT's `sub` claim** (fall back to `oid`, then to the anonymous fallback — `actor_id(AGENT_ID)`, i.e. `"order-triage"` — when no user token is present). The CUSTOM_JWT authorizer has already cryptographically verified the token before the runtime sees it (per ADR-0001), so the runtime decodes the **payload only, without re-verifying the signature**, purely to read the subject. `sub` is an opaque, stable per-(user, app) identifier — it keeps **PII out of the namespace path** (no email/UPN) while giving each user their own `/facts` and `/preferences` partition. The agent's `runtime.py` resolves the actor **once** from the request `identity` contextvar (`identity.actor_id(AGENT_ID)`, with `identity.ANONYMOUS_ACTOR` as the module-level constant) and threads it as the explicit `actor_id` argument into `build_agent`, which passes it on to `build_session_manager` — a single source for both the memory partition and the Converse `requestMetadata`.
...
2. [x] **D2** — JWT-payload subject decode in `agent_kit.infra.identity` (`sub`→`oid`→anonymous); resolved once in the agent's `runtime.py` and threaded `runtime.py → build_agent → build_session_manager`.
